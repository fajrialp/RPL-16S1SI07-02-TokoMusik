  ...
  ?>
    <div class="struk"><!-- class struk -->
      <form name="struk" method="POST" action="">
        <table class="form">
          <?php 
  		  $query     = mysqli_query($koneksi, "SELECT * FROM toko LIMIT 1");
  		  $jml_field = mysqli_num_rows($query);
  		  if ($jml_field == 1){
        ...
        <table class="form">
          <?php
		  $trans=mysqli_query($koneksi, "SELECT transaksi_jual.*,
							 		  barang.* 
							   FROM transaksi_jual,
							   		 barang 
							   WHERE transaksi_jual.id_brg=barang.id_brg
							     AND aktif=1 
							ORDER BY nomor");		  
		  while ($tr=mysqli_fetch_array($trans)){
		  ?>
          <tr>
