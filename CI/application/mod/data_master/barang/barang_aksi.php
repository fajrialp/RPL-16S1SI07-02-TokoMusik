...
<?php
  ...
  if ($_SESSION['status']=='admin'){
    ...
    if ($mod=='brg' AND $aksi=='hap_dt'){
      mysqli_query($koneksi, "DELETE FROM barang WHERE id_brg='$_GET[id]'");
      header('location:../../../mediaweb.php?mod='.$mod);
    }
    //tambah data
    elseif($mod=='brg' AND $aksi=='tb_dt'){
      mysqli_query($koneksi, "INSERT INTO barang(id_brg,
									  id_merk,
									  id_kat,
									  id_sat,
									  nama_brg,
    ...
    elseif($mod=='brg' AND $aksi=='ubh_dt'){
      mysqli_query($koneksi, "UPDATE barang SET nama_brg   = '$_POST[nama_brg]',
									 id_merk    = '$_POST[merk]',
